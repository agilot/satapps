...
//         And(Variable(ts._1), Prop.And(ts._2.toList.map(clauseConv)))
//     }
//   }
// }
...
//   def And(ex : Iterable[Expr]) = ex.fold(T)(And(_, _))
//   def orAll(ex: Iterable[Expr]) = ex.fold(F)(Or(_, _))

//   def all(v: Iterable[Variable]): Expr = And(v)
//   def none(v: Iterable[Variable]): Expr = And(v.map(Not(_)))
...
//     val n = v.size
//     if (k >= n) T
//     else if (k == 0) And(v.map(va => Not(va)))
//     else 
//       val it = v.zip(Range(0, n))
//       val it2 = if(n >= 2) it.tail.init else Nil
//       val o1 = Or(Not(v.head), reg(0, 0))
//       val o2 = if (k < 2) T else And((for(j <- 1 until k) yield Not(reg(0, j))).toList)
//       val o3 = if (n < 3) T else And((for((vi, i) <- it2) yield Or(Not(vi), reg(i, 0))).toList)
//       val o4 = if (n < 3) T else And((for((vi, i) <- it2) yield Or(Not(reg(i - 1, 0)), reg(i, 0))).toList)
//       val o5 = if (n < 3 || k < 2) T else And((for((vi, i) <- it2; j <- 1 until k) yield Or(Not(vi), Or(Not(reg(i - 1, j - 1)), reg(i, j)))).toList)
//       val o6 = if (n < 3 || k < 2) T else And((for((vi, i) <- it2; j <- 1 until k) yield Or(Not(reg(i - 1, j)), reg(i, j))).toList)
//       val o7 = if (n < 2) T else And((for((vi, i) <- it.tail) yield Or(Not(vi), Not(reg(i - 1, k - 1)))).toList)
//       And(o1 :: o2 :: o3 :: o4 :: o5 :: o6 :: o7 :: Nil)
...
//         case x :: xs => 
//           And(And(xs.map(y => Or(Not(x), Not(y)))), notPairs(xs))
//         case _ => throw IllegalArgumentException("This state should never occur")
//     v match{
...
//       Graph((0 until s.size * 2).toSet, gen2SATEdgeSet(e)).transClos
